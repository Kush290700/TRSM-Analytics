...
def generate_favicon_T_netflix_style(output_path):
    # Canvas size
    size = 256
    ...
    # Text content
    text = "T"
    text_color = (0, 0, 0, 255) # Deep Black

    # Font logic
    font_candidates = ['Impact', 'Arial Black', 'Roboto-Black', 'Montserrat-ExtraBold']
    ...
    if font_file:
        ...
        try:
            ...
            # Find the largest font size that fits 95% of the canvas width and height
            while True:
                # Check current size
                ...
                # Check if it's within 95% of both width and height
                if text_width < size * 0.95 and text_height < size * 0.95:
                    ...
                    try:
                        ...
                        test_text_height = test_bbox[3] - test_bbox[1]
                        if test_text_width <= size * 0.95 and test_text_height <= size * 0.95:
                            font_size = test_font_size
                            selected_font = test_font
                        else: # Can't increase more without exceeding 95%
                            break
                    except IOError:
                        break # Should not happen if font_file is valid
                else: # Optimal size found or already too big
                    break
        except IOError:
...
if __name__ == "__main__":
    output_favicon_path = "app/static/images/favicon_T_netflix_style.png"
    generate_favicon_T_netflix_style(output_favicon_path)
